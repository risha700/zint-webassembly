export default class BarcodeGenerator{
    constructor(processor_path='./zint-processor.js', wrapper_class){
        ...
        this.bmp_data = null
        this.img = document.getElementsByTagName('img')[0]|| new Image();
        this.output_wrapper = document.createElement('div')
        this.download_btn = document.createElement('button')
        this.print_btn = document.createElement('button')
    ...
    initEvents(){
        ...
        this.download_btn.addEventListener('click',()=>{
            window.URL.revokeObjectURL(this.prepareDownloadUrl())
        })
        this.print_btn.addEventListener('click', ()=>{
            let popup = window.open(this.img.src,"_new")
    ...
    buildUI(){
        ...
        this.download_btn.innerHTML = "&#x1f4be;"
        this.print_btn.innerHTML = "&#128424;"
        ...
        this.output_wrapper.setAttribute('class', 'output_wrapper')
    }
    initWorker(){
    ...
    drawBmpArr(arr, data){
        ...
        this.img.src = url;
        this.showOutputFrame(this.img, data)

    }
    prepareDownloadUrl(){
        let url = window.URL.createObjectURL(new Blob([this.bmp_data], {type: "image/octet-stream"}))
        let a = document.createElement('a')
        a.setAttribute("href", url);
        a.setAttribute("download", `${this.barcodeText.value}.png`)
        a.click()
        return url
    }
    showOutputFrame(img, data){
        this.output_wrapper.appendChild(img)   // needs a change 
        this.output_wrapper.appendChild(this.download_btn)
        this.output_wrapper.appendChild(this.print_btn)
    ...
    hasCErrors(data){
        ...
        let errors = data[0][4]
        this.safeReset(this.errorText)
        this.safeReset(this.output_wrapper)
        if(errors && errors.length){
            this.errorText.innerText = errors;
    ...
    safeReset(elm){
        try{this.wrapper.removeChild(elm) }catch{}
    }
